...
# def browse():
#     global filename
#     filename=filedialog.askopenfile(initialdir=os.getcwd(),title="select pdf file",filetypes=(("PDF FILE","*pdf"),("all files","*.*")))
...
def protect():
    ...
    if pas=="" and explor=="" and rename=="":
        ...
        messagebox.showinfo("Erorr","Enter Location Please.")
    elif rename=="":
        ...
        try:
            out=PdfFileWriter()
            file=PdfFileReader(open(f"{home_directory}\\Desktop\\{explor}.pdf","+rb"))
            print(file)
            num=file.numPages
            for i in range(num):
             page=file.getPage(i)
...
lbl0=tk.Label(text="Note:Please But Your files on Desktop..                                 Made By: AHMED RAMADAN",fg="white",bg="black",font=("arail,20,bold")).place(x=0,y=205)
